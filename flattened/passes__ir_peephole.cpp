...
void peephole_basic_block(BasicBlockPtr basic_block, Builder& builder) {
  ...
  while (curr_instruction != basic_block->tail_instruction) {
    ...
    if (std::holds_alternative<instruction::Binary>(curr_instruction->kind)) {
      const auto& curr_kind = std::get<instruction::Binary>(curr_instruction->kind);
      ...
      if (std::holds_alternative<instruction::Binary>(next_instruction->kind)) {
        const auto& next_kind = std::get<instruction::Binary>(next_instruction->kind);
        ...
        && curr_dst->use_id_list.size() == 1) {
          ...
          auto instruction = builder.fetch_binary_instruction(
              ...
              next_dst->id,
              curr_lhs->id,
              builder.fetch_constant_operand(builder.fetch_i32_type(), (int)(constant_value0 + constant_value1))
          );
      ...
      } else if (curr_op == instruction::BinaryOp::Mul 
        && curr_lhs->is_int()
        && curr_rhs->is_int()
        && curr_rhs->is_constant()) {
        ...
        && curr_rhs->is_constant()) {
          ...
          if (constant_value == 1) {
            ...
            for (auto use_instruction_id : use_id_list_copy) {
                auto instruction = builder.context.get_instruction(use_instruction_id);
                instruction->replace_operand(curr_dst->id, curr_lhs->id, builder.context);
            }
            curr_instruction->remove(builder.context);
            ...
            curr_instruction->insert_next(builder.fetch_binary_instruction(
                ...
                curr_dst->id,
                tmp1,
                builder.fetch_constant_operand(builder.fetch_i32_type(), (int)constant_value)
            ));
            curr_instruction->insert_next(builder.fetch_binary_instruction(
                instruction::BinaryOp::Add,
                tmp1,
        ...
        && curr_rhs->is_constant()) {
          ...
          if (constant_value > 1 && (constant_value & (constant_value - 1)) == 0) {
            ...
            curr_instruction->insert_next(builder.fetch_binary_instruction(
                instruction::BinaryOp::And,
                tmp3,
                tmp2,
                builder.fetch_constant_operand(builder.fetch_i32_type(), (int)constant_value - 1)
            ));
            curr_instruction->insert_next(builder.fetch_binary_instruction(
                instruction::BinaryOp::Add,
                tmp2,
    ...
    //   auto curr_kind = std::get<instruction::GetElementPtr>(curr_instruction->kind);
    //   auto curr_dst = curr_kind.dst_id;
    //   auto curr_ptr = curr_kind.ptr_id;
    //   for (auto &curr_operand : curr_kind.index_id_list) {
