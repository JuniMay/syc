...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      // INTEGER
      // FLOATING
      char dummy1[sizeof (ComptimeValuePtr)];
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_FLOATING: // FLOATING
        value.move< ComptimeValuePtr > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, ComptimeValuePtr&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const ComptimeValuePtr& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      case symbol_kind::S_INTEGER: // INTEGER
      case symbol_kind::S_FLOATING: // FLOATING
        value.template destroy< ComptimeValuePtr > ();
        break;
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {}
#if 201103L <= YY_CPLUSPLUS
      symbol_type (int tok, ComptimeValuePtr v, location_type l)
        : super_type (token_type (tok), std::move (v), std::move (l))
#else
      symbol_type (int tok, const ComptimeValuePtr& v, const location_type& l)
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_INTEGER (ComptimeValuePtr v, location_type l)
      {
      ...
      static
      symbol_type
      make_INTEGER (const ComptimeValuePtr& v, const location_type& l)
      {
      ...
      static
      symbol_type
      make_FLOATING (ComptimeValuePtr v, location_type l)
      {
      ...
      static
      symbol_type
      make_FLOATING (const ComptimeValuePtr& v, const location_type& l)
      {
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      case symbol_kind::S_INTEGER: // INTEGER
      case symbol_kind::S_FLOATING: // FLOATING
        value.copy< ComptimeValuePtr > (YY_MOVE (that.value));
        break;
   ...
   Parser ::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      case symbol_kind::S_INTEGER: // INTEGER
      case symbol_kind::S_FLOATING: // FLOATING
        value.move< ComptimeValuePtr > (YY_MOVE (s.value));
        break;
...
#line 2086 "frontend/generated/parser.h"
