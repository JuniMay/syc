...
void peephole_basic_block(BasicBlockPtr basic_block, Builder& builder) {
  ...
  while (curr_instruction != basic_block->tail_instruction) {
    ...
    if (std::holds_alternative<BinaryImm>(curr_instruction->kind)) {
      ...
      if (rd->is_vreg() && rs->is_vreg() && imm->is_zero() && (op == BinaryImm::ADDI || op == BinaryImm::ADDIW)) {
        ...
        if (std::holds_alternative<Load>(next_instruction->kind)) {
          auto next_kind = std::get<Load>(next_instruction->kind);
        ...
        } else if (std::holds_alternative<Store>(next_instruction->kind)) {
          auto next_kind = std::get<Store>(next_instruction->kind);
    ...
    if (std::holds_alternative<Lui>(curr_instruction->kind)) {
      // remove: lui v0, 0
      // make all the uses of v0 use zero instead
      const auto& kind = std::get<Lui>(curr_instruction->kind);
      auto rd = builder.context.get_operand(kind.rd_id);
      auto imm = builder.context.get_operand(kind.imm_id);
    ...
    if (std::holds_alternative<Binary>(curr_instruction->kind)) {
      // remove: mul v0, zero, v1
      //         mul v0, v1, zero
      // remove: add/sub(w) v0, zero, v1
      //         add/sub(w) v0, v1, zero
      const auto& kind = std::get<Binary>(curr_instruction->kind);
      auto op = kind.op;
      auto rd = builder.context.get_operand(kind.rd_id);
      ...
      } else if (rd->is_vreg() && rs2->is_zero() && (op == Binary::ADD || op == Binary::ADDW)) {
        if (rd->use_id_list.size() == 1) {
          auto use_id_list_copy = rd->use_id_list;
          for (auto use_instruction_id : use_id_list_copy) {
            auto instruction =
