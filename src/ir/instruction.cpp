      ...
      overloaded{
          ...
          [&context](const Ret& instruction) -> std::string {
            if (instruction.maybe_value_id.has_value()) {
              auto value =
                  context.get_operand(instruction.maybe_value_id.value());
          ...
          [&context](const CondBr& instruction) {
            ...
            auto true_block =
                context.get_basic_block(instruction.then_block_id);
            auto false_block =
                context.get_basic_block(instruction.else_block_id);
          ...
          [&context](const Alloca& instruction) {
            auto dst_str = context.get_operand(instruction.dst_id)->to_string();
            auto allocated_type_str =
                type::to_string(instruction.allocated_type);
            auto result = dst_str + " = alloca " + allocated_type_str;

            if (instruction.maybe_size_id.has_value()) {
              auto size =
                  context.get_operand(instruction.maybe_size_id.value());
          ...
          [&context](const Call& instruction) {
            ...
            auto result = "call " + return_type_str + " @" +
            ...
            if (instruction.maybe_dst_id.has_value()) {
              auto dst_str =
                  context.get_operand(instruction.maybe_dst_id.value())
                      ->to_string();
